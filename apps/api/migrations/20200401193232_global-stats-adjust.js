    ...
    return knex.raw(`
        CREATE OR REPLACE VIEW global_stats AS
            SELECT
                ...
                sum(case when request.status = 'MANAGEMENT_BUILDING' then request.mask_small_size_quantity + request.mask_large_size_quantity else 0 end)
                -
                sum(request.mask_small_size_delivered_quantity + request.mask_large_size_delivered_quantity) as in_production_shade_qty,
