exports.up = async function (knex) {
    ...
    return knex.raw(`
        ...
        SELECT
            ...
            sum(
                case when request.status in ('MANAGEMENT_BUILDING','MANAGEMENT_DELIVERED')
                then request.mask_small_size_quantity + request.mask_large_size_quantity
                else 0
                end
            )
            ...
            sum(
                case when request.status in ('MANAGEMENT_BUILDING','MANAGEMENT_DELIVERED')
                then request.mask_small_size_delivered_quantity + request.mask_large_size_delivered_quantity
                else 0
                end
