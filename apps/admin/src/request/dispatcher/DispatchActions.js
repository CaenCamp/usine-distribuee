...
const DispatchActions = ({ record }) => {
    ...
    const disabled = queryLoading || dispatchLoading || postoneLoading || rejectLoading || toDispatchLoading;

    const handleProductionManagerChange = (evt) => {
        setProductionManager(evt.target.value);
    }

    if (record.status === 'DISPATCH_TODO' || record.status === 'DISPATCH_PENDING') {
        return (
            <Grid container className={classes.container}>
                {record.status === 'DISPATCH_TODO' && <Grid item xs={3}>
                ...
                <Grid item xs={2}>
                    <Button variant="contained" color="primary" disabled={disabled} onClick={reject}>
                        Rejeter
                    </Button>
                </Grid>
                <Grid item xs={7} justify="flex-end" alignItems="flex-start" container>
